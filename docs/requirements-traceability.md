...
FR/SC来自v1.2。成员C的前端与模拟测试已加入，下列In progress仅表示部分前端实现；完整链上/后端要求未Verified。证据见成员C交付指南。

| requirementId | 原文位置与摘要 | owner | plannedLocation | implementationFiles | testIds | status | commit | evidenceLink |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| FR-01 | PRD 6/7章：界面始终显示“Monad 测试网 · 测试资产，无现金价值” | C | apps/web/src/features/ | apps/web/src/components/; apps/web/src/features/; packages/shared/src/money.ts | AT41 | In progress | — | [C前端证据](member-c-delivery.md) |
| FR-02 | PRD 6/7章：本版押金 D 范围为 1—10,000 MockUSD，前端输入最多 2 位小数，合约接受以 0.01 MockUSD 为最小业务单位的金额 | B/C | contracts/src/; packages/shared/src/ | apps/web/src/components/; apps/web/src/features/; packages/shared/src/money.ts | AT02/10/36 | In progress | — | [C前端证据](member-c-delivery.md) |
...
| FR-05 | PRD 6/7章：分配形成“可领取余额”；领取交易确认后才是“已领取” | B/C | contracts/src/; apps/web/src/features/ | apps/web/src/components/; apps/web/src/features/; packages/shared/src/money.ts | AT03/12/23 | In progress | — | [C前端证据](member-c-delivery.md) |
| FR-06 | PRD 6/7章：房东创建可编辑草稿，填写租赁关系和约定，选择有效服务方案 | C/D | apps/web/src/app/; apps/web/src/server/ | apps/web/src/components/; apps/web/src/features/; packages/shared/src/money.ts | AT27/41/52 | In progress | — | [C前端证据](member-c-delivery.md) |
| FR-07 | PRD 6/7章：房东检查条款后部署租约合约 | B/C | contracts/src/; apps/web/src/features/ | apps/web/src/components/; apps/web/src/features/; packages/shared/src/money.ts | AT01/02/37/38/40 | In progress | — | [C前端证据](member-c-delivery.md) |
| FR-08 | PRD 6/7章：租客先查看完整金额、规则和收款合约，再确认存入 | B/C | contracts/src/; apps/web/src/features/ | apps/web/src/components/; apps/web/src/features/; packages/shared/src/money.ts | AT03/41/43 | In progress | — | [C前端证据](member-c-delivery.md) |
| FR-09 | PRD 6/7章：入金前任一当事人可取消该约定；取消后不能再接受或入金 | B | contracts/src/ | — | AT01/02/26 | Not started | — | — |
| FR-10 | PRD 6/7章：入住清单至少包含房间／物品名称、状况描述、文件、提交者、提交时间和对方状态 | C/D | apps/web/src/features/; apps/web/src/server/ | apps/web/src/components/; apps/web/src/features/; packages/shared/src/money.ts | AT27/33/35 | In progress | — | [C前端证据](member-c-delivery.md) |
| FR-11 | PRD 6/7章：每次提交形成新版本，已提交的正文和文件不能被覆盖 | B/D | contracts/src/; apps/web/src/server/ | — | AT33/35 | Not started | — | — |
| FR-12 | PRD 6/7章：维修记录、租期内事件和退租清单复用该结构 | C/D | apps/web/src/features/; apps/web/src/server/ | apps/web/src/components/; apps/web/src/features/; packages/shared/src/money.ts | AT33/35 | In progress | — | [C前端证据](member-c-delivery.md) |
| FR-13 | PRD 6/7章：任一方可请求确认提前交接，提交实际日期和材料摘要；另一方认可或有效处理结果确认后，可提前开启申索 | B/C | contracts/src/ | apps/web/src/components/; apps/web/src/features/; packages/shared/src/money.ts | AT05/06/45/50 | In progress | — | [C前端证据](member-c-delivery.md) |
...
| FR-16 | PRD 6/7章：申索窗口内，房东可以准备最多 10 项扣款，一次性在链上提交最终清单；每项必须金额为正、带证据清单承诺值，累计金额不超过 D | B/C | contracts/src/ | apps/web/src/components/; apps/web/src/features/; packages/shared/src/money.ts | AT10/11/26 | In progress | — | [C前端证据](member-c-delivery.md) |
| FR-17 | PRD 6/7章：扣款的私有正文包含类别、金额、理由、对应租约条款、入住和退租资料引用、票据／报价和解释 | C/D | apps/web/src/features/; apps/web/src/server/ | apps/web/src/components/; apps/web/src/features/; packages/shared/src/money.ts | AT11/27/33 | In progress | — | [C前端证据](member-c-delivery.md) |
...
| FR-20 | PRD 6/7章：租客可在清单提交后至统一 responseDeadline 前，逐项“认可”或“提出异议”；认可需要单独确认金额及不可撤销性 | B/C | contracts/src/ | apps/web/src/components/; apps/web/src/features/; packages/shared/src/money.ts | AT12/13/26 | In progress | — | [C前端证据](member-c-delivery.md) |
...
| FR-25 | PRD 6/7章：证据窗口结束后，R 可以提出一次结果 | B/C | contracts/src/ | apps/web/src/components/; apps/web/src/features/; packages/shared/src/money.ts | AT16/36 | In progress | — | [C前端证据](member-c-delivery.md) |
| FR-26 | PRD 6/7章：R 提出结果后，T、L 任一方可在 T_challenge 内请求备用处理，附理由承诺值 | B/C | contracts/src/ | apps/web/src/components/; apps/web/src/features/; packages/shared/src/money.ts | AT16/17/26 | In progress | — | [C前端证据](member-c-delivery.md) |
...
| FR-30 | PRD 6/7章：成功入金后、U>0 且尚未达到有效 timeoutAt/hardEndAt 时，双方可对当前未分配金额共同和解 | B/C | contracts/src/ | apps/web/src/components/; apps/web/src/features/; packages/shared/src/money.ts | AT21/22/49 | In progress | — | [C前端证据](member-c-delivery.md) |
...
| FR-32 | PRD 6/7章：U=0 时显示“分配完成”；U+CT+CL=0 时显示“全部领取完成” | B/C | contracts/src/; apps/web/src/features/ | apps/web/src/components/; apps/web/src/features/; packages/shared/src/money.ts | AT23/46/47 | In progress | — | [C前端证据](member-c-delivery.md) |
...
| FR-34 | PRD 6/7章：正式提交的材料按清单打包，由提交者钱包调用 recordEvidence 记录 bundleId、版本和承诺值；对方调用 acknowledgeEvidence 表示认可或异议 | B/C/D | contracts/src/; apps/web/src/server/ | apps/web/src/components/; apps/web/src/features/; packages/shared/src/money.ts | AT33/35 | In progress | — | [C前端证据](member-c-delivery.md) |
